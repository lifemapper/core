#!/bin/bash
"""
@summary: This script attempts to generate an openModeller model from the given
             Lifemapper inputs
@author: CJ Grady
@version: 4.0.0
@status: beta
...
"""
import argparse
from LmCompute.plugins.sdm.openModeller.omRunners import OMModelRunner
...
if __name__ == "__main__":
   ...
   parser = argparse.ArgumentParser(
      description="This script attempts to generate an openModeller model from the given Lifemapper inputs") 
   ...
   job = OMModelRunner(args.jobXml, jobName=args.jobName, outDir=args.outDir, 
                       workDir=args.workDir, metricsFn=args.metricsFn, 
                       logFn=args.logFn, logLevel=args.logLevel, 
                       statusFn=args.statusFn)
   job.run()
