...
def testExperiment():
   ...
   lyr1Fn = os.path.join(SAMPLE_DATA_PATH, "Gulo gulo Projection 2512508.tif")
   
   # layer 2 - some other raster
   lyr2Name = "Rana areolata %s" % gmt().mjd
   lyr2Fn = os.path.join(SAMPLE_DATA_PATH, "Rana areolata Projection 2515034.tif")
   
   # layer 4 - vector layer 1-2
   lyr4Name = "Peromyscus maniculatus %s" % gmt().mjd
   lyr4Fn = os.path.join(SAMPLE_DATA_PATH, "Peromyscus_maniculatus.shp")
   
   # layer 5 - vector layer
   lyr5Name = "Procyon lotor %s" % gmt().mjd
   lyr5Fn = os.path.join(SAMPLE_DATA_PATH, "Procyon_lotor.shp")
   
   # layer 6 - vector layer
   lyr6Name = "Tamiasciurus hudsonicus %s" % gmt().mjd
   lyr6Fn = os.path.join(SAMPLE_DATA_PATH, "Tamiasciurus_hudsonicus.shp")
