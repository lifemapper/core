...
BEGIN
   -- if layer is found
   SELECT count(*) INTO temp FROM lm3.scenario WHERE scenarioid = scenid;
   IF temp < 1 THEN
      RAISE EXCEPTION 'Scenario with id % does not exist', scenid;
   END IF;
   
   SELECT count(*) INTO temp FROM lm3.layer WHERE layerid = lyrid;
   IF temp < 1 THEN
      RAISE EXCEPTION 'Layer with id % does not exist', lyrid;
   END IF;
   
   SELECT count(*) INTO temp FROM lm3.ScenarioLayers WHERE scenarioId = scenid AND layerId = lyrid;
   IF temp < 1 THEN
      -- get or insert scenario x layer entry
      INSERT INTO lm3.ScenarioLayers (scenarioId, layerId) VALUES (scenid, lyrid);
      IF FOUND THEN
         success := 0;
      END IF;
   ELSE
...
CREATE OR REPLACE FUNCTION lm3.lm_test(scenid int)
   RETURNS int AS
$$
DECLARE
   success int = -1;
...
BEGIN
   ...
   IF NOT FOUND THEN
      RAISE NOTICE 'not found: tmpcount1 = %, FOUND = %', tmpcount1, FOUND;
   ELSE
      RAISE NOTICE 'found: tmpcount1 = %, FOUND = %', tmpcount1, FOUND;
   END IF;
